 ...
 * вместо new PersonMapper ()) мы написали new BeanPropertyRowMapper<>(Person.class)) */
    ...
    public Optional <Person> show (String email, int id ) {
        return jdbcTemplate.query("SELECT * FROM Person WHERE email = ? and id != ?",
                new Object[] {email, id},
    ...
    public void testMultipleUpdate (){
     ...
     /** метод для быстрого создания 1000 новых записей Person в базе данных */
    private List <Person> create1000People() {
        List <Person> people = new ArrayList<>();

        /** генерируем 1000 новых людей */
